...
sap.ui.define([
	...
	opaTest("Should see the table with all entries", function (Given, When, Then) {
		...
		Then.onTheWorklistPage.iSeeTable().and.iSeeTableHasData().and.tableHeaderDisplayTotal();
	});
	
	opaTest("Search one has result", function(Given, When, Then){
		When.onTheWorklistPage.iSearchForSomethingHasResult();
		
		Then.onTheWorklistPage.iShouldSearchAndSeeResult();
	});
	
	opaTest("Click Share Button and see two buttons", function(Given, When, Then){
		When.onTheWorklistPage.iPressOnShareButton();
		
		Then.onTheWorklistPage.iShouldSeeTheShareEmailButton().and.iShouldSeeTheShareTileButton();
	});
