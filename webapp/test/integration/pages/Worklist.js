sap.ui.define([
	...
	"./Common",
	"./shareOptions"
], function (Opa5, Press, EnterText, AggregationFilled, AggregationEmpty, Ancestor, I18NText, AggregationLengthEquals, PropertyStrictEquals, Common, shareOptions) {
	"use strict";
	...
	Opa5.createPageObjects({
		...
		onTheWorklistPage: {
			...
			actions: Object.assign({
					...
					return this.waitFor(createWaitForItemAtPosition({
						...
						success: function (oTableItem) {
							...
							this.getContext().currentItem = {
								...
								id: oBindingContext.getProperty("ProjectUUID"),
								name: oBindingContext.getProperty("Projdef")
							};
						}
					}));
				},
				...
				//4 搜索Projdef 2，应该显示该条数据结果
				iSearchForSomethingHasResult: function() {
					return this.waitFor({
						id: sSearchFieldId,
						actions: [
							new EnterText({
				...
				//6 点击分享按钮，应该显示两个按钮（发送和收藏）
				
				iPressOnShareButton: function () {
					return this.waitFor({
			...
			assertions: Object.assign({
				...
				//3 表头应该显示数量10
				tableHeaderDisplayTotal: function() {
					return this.waitFor({
						id: "tableHeader",
						...
						success: function() {
							Opa5.assert.ok(true, "The table header has total");
						},
						errorMessage: "The table header does not contain total"
					});	
				},
				//4 搜索Projdef 2，应该显示该条数据结果
				iShouldSearchAndSeeResult: function() {
					return this.waitFor({
						id: sTableId,
							viewName: sViewName,
						...
						success: function (oTable) {
							return this.waitFor({
								...
								success: function (aItems) {
									var aAllEntities = this.getEntitySet("ZDEMO_C_Pro_TP_D");
